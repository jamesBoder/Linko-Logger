...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || user == "" {
		httpError(r.Context(), w, http.StatusUnauthorized, pkgerr.New("user not authenticated"))
		return
	}
...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			...
			httpError(r.Context(), w, http.StatusInternalServerError, pkgerr.New("failed to lookup URL"))
		}
		return
	}
